...
func getProductDataFromMagento(tokens, serviceURL string) (string, int, error) {
	// get token
...
func GetProducts(tokens string) (string, int, error) {
	serviceURL := os.Getenv("MAGE_HOST") + "/rest/all/V1/products?searchCriteria[currentPage]=1"

	// fmt.Println("GetProductAll :: ServiceURL => ", serviceURL)

	return getProductDataFromMagento(tokens, serviceURL)
}

func GetProductBySKU(tokens, sku string) (string, int, error) {
	escapedSKU := url.QueryEscape(sku)
	serviceURL := os.Getenv("MAGE_HOST") + "/rest/all/V1/products/" + escapedSKU

	// fmt.Println("GetProductBySKU :: ServiceURL => ", serviceURL)
